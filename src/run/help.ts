...
export function buildDaemonHelp(): string {
  return [
    ...
    "",
    "Commands:",
    "  install   Install/upgrade daemon autostart, config, and the Chrome native messaging host",
    "  restart   Restart the daemon autostart service",
    "  status    Check daemon service, native host, and daemon health",
    "  uninstall Remove daemon autostart and the Chrome native messaging host",
    ...
    "  --extension-id   Dev-only unpacked Chrome extension ID (requires --dev)",
