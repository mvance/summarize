...
- Antigravity normal text summaries run `agy --print <prompt>` in a temporary cwd with `--sandbox`. Attachment prompts keep the caller cwd so agy can inspect the requested path but do not auto-approve tools.
